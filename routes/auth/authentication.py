...
async def create_password(
    ...
    try:
        ...
        if len(password.password) > 51 or len(password.password) < 5:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={
                    "message": "Password must be between 5 and 51 characters long.",
                    "success": SUCCESS.FALSE,
                },
            )
...
async def sing_in(db: db_dependencies, user_info: SignIn, request: Request):
    ...
    try:
        ...
        if len(user_info.password) > 51 or len(user_info.password) < 5:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={
                    "message": "Password must be between 5 and 51 characters long.",
